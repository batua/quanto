...
def groupby_subplot(grouped, columns, plot, ncol=4, figsize=(20,15), ticksize=(8,8)):
    """
    ...
    with plt.rc_context({'xtick.labelsize': xticksize, 'ytick.labelsize': yticksize}):
        fig, axes = plt.subplots(int(ngroups/ncol) + 1 * (ngroups % ncol > 0), ncol, figsize=figsize, squeeze=False)
...
def groupby_asdf(df, grouped, idx=0):
    """
    Returns pandas DataFrame as a subset from df with given index idx in the grouped
    ...
    Returns
    -------
    pandas DataFrame as a subset from df with given index idx in the grouped
